...
detect_gpu_vendor() {
    local pci_info
    pci_info=$(lspci 2>/dev/null | grep -Ei 'vga|3d controller' || true)
    if echo "$pci_info" | grep -qi nvidia; then
        echo "nvidia"
    elif echo "$pci_info" | grep -Eqi 'amd|ati|radeon'; then
        echo "amd"
    else
        echo "unknown"
    fi
...
case "$GPU_VENDOR" in
    nvidia) log "Detected GPU vendor: NVIDIA -> will set up Rigel." ;;
    amd)    log "Detected GPU vendor: AMD -> will set up TeamRedMiner." ;;
    *)      warn "Couldn't auto-detect GPU vendor from lspci. If you pick GPU mining below, you'll be asked to specify NVIDIA or AMD manually." ;;
esac
echo ""
...
if $INSTALL_GPU && [[ "$GPU_VENDOR" == "unknown" ]]; then
    echo ""
    echo "Which GPU do you have?"
    echo "  1) NVIDIA"
    echo "  2) AMD"
    read -rp "Enter 1 or 2: " VENDOR_CHOICE
...
# ---------------------------------------------------------------------
# 3. GPU driver / runtime setup (branches by vendor)
# ---------------------------------------------------------------------
if $INSTALL_GPU && [[ "$GPU_VENDOR" == "nvidia" ]]; then
...
if $INSTALL_GPU && [[ "$GPU_VENDOR" == "amd" ]]; then
    log "AMD GPU detected - the amdgpu kernel driver is already built into the Linux kernel,"
    log "so no separate proprietary driver install is needed (unlike NVIDIA)."
    log "Installing OpenCL runtime packages needed by TeamRedMiner..."
    sudo dnf install -y mesa-libOpenCL ocl-icd ocl-icd-devel clinfo

    log "Checking OpenCL can see your GPU..."
    if clinfo 2>/dev/null | grep -qi "Device Name"; then
        log "OpenCL device detected."
    else
        warn "clinfo didn't report a device. TeamRedMiner may still work, but if it can't see"
        warn "your GPU, you may need ROCm's full OpenCL stack instead of Mesa's - see:"
        warn "https://github.com/todxx/teamredminer for driver requirements per GPU generation."
    fi
fi
...
if $INSTALL_GPU; then
    if [[ "$GPU_VENDOR" == "nvidia" ]]; then
        ...
        download_latest_release "TeamRedMiner" "todxx" "teamredminer" "linux.*\\.tgz$" "$BIN_DIR/teamredminer" "teamredminer" || true
    fi
fi
...
if [[ ! -f "$CONFIG_PATH" ]]; then
    ...
    python3 - "$SCRIPT_DIR/config.example.json" "$CONFIG_PATH" "$BTC_ADDRESS" "$WORKER_NAME" "$INSTALL_GPU" "$INSTALL_CPU" "$GPU_VENDOR" <<'PYEOF'
import json, sys

example_path, out_path, btc_address, worker_name, install_gpu, install_cpu, gpu_vendor = sys.argv[1:8]
...
config["gpu"]["vendor"] = gpu_vendor if gpu_vendor in ("nvidia", "amd") else "auto"
config["gpu"]["wallet_nvidia"] = f"XEL:{btc_address}"
config["gpu"]["wallet_amd"] = f"RVN:{btc_address}"
